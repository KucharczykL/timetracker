...
# `test-fast` and `test-e2e` each pin a directory. A word holding `/` is a
# path, so it replaces the pinned directory; a bare flag and its value (`-k relation`) hold none and
