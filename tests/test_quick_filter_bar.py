...
from common.components import QUICK_FACETS, QuickFilterBar, is_quick_editable
...
class QuickFacetsContractTest(TestCase):
    ...
    def test_every_facet_is_an_own_model_set_field(self):
        for mode, facets in QUICK_FACETS.items():
            ...
            for facet in facets:
                with self.subTest(mode=mode, facet=facet.field):
                    ...
                    self.assertEqual(metadata[facet.field]["kind"], "set")
