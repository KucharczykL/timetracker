...
def test_percent_bearing_value_round_trips_through_the_real_reader(db, clean_ini_env):
    ...
    SiteSetting.objects.create(key="DEFAULT_PURCHASE_CURRENCY", value="EU%R")

    text = export_site_settings_ini()
    clean_ini_env.write_text(text)
    config_module.reset_caches()

    result = resolve_raw_with_source("DEFAULT_PURCHASE_CURRENCY")
...
def test_exported_values_are_never_quoted(db):
    ...
    line = next(
        ...
        if line.startswith("DEFAULT_PURCHASE_CURRENCY")
    )
    assert '"' not in line
    assert "'" not in line
