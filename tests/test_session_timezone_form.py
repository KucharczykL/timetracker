...
def _form_data(game: Game, **overrides) -> dict[str, str]:
    data = {
        ...
        "started_at": "2026-07-01T21:00:00+09:00",
...
def test_zone_binds_to_the_statement_and_empty_cleans_to_null(library):
    game = Game.objects.create(library=library, name="Hades")
    form = SessionForm(
        data=_form_data(game), library=library, presentation=_PRESENTATION
    )
    assert form.is_valid(), form.errors
    timing = form.timing_statement("Europe/Prague")
    assert timing.started_at_zone == "Asia/Tokyo"
    assert timing.ended_at_zone is None
...
def test_invalid_zone_is_a_form_error(library):
    ...
    form = SessionForm(
        library=library,
        data=_form_data(game, started_at_zone="Not/AZone"),
        presentation=_PRESENTATION,
    )
    assert not form.is_valid()
    assert "started_at_zone" in form.errors
...
def test_an_end_timestamp_on_the_add_form_captures_its_zone_too(library):
    ...
    form = SessionForm(
        library=library,
        presentation=_PRESENTATION,
        initial={"ended_at": datetime(2026, 7, 1, 14, 0, tzinfo=UTC)},
    )
    html = render(FormFields(form, embedded=SESSION_TIMEZONE_EMBEDS))
...
def test_embedded_field_renders_in_its_host_row_not_its_own(library):
    ...
    assert "Started at zone" not in html
    ...
    assert html.count(' name="started_at_zone"') == 1
