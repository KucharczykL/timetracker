...
def test_choice_vocabulary_matches_single_vocabulary_call() -> None:
    # render_choice_vocabulary is a thin wrapper over render_choice_vocabularies
    # with one vocab — output must stay byte-identical for existing consumers.
    single = render_choice_vocabulary(
        type_name="ThemePreference",
        values_name="THEME_PREFERENCES",
        labels_name="THEME_LABELS",
        choices=(("system", "System"), ("light", "Light")),
    )
    multi = render_choice_vocabularies(
        [
            ChoiceVocab(
                "ThemePreference",
                "THEME_PREFERENCES",
...
def test_choice_vocabularies_combines_multiple_into_one_module() -> None:
    output = render_choice_vocabularies(
        [
            ChoiceVocab(
                "SettingNamespace",
                "SETTING_NAMESPACES",
                "SETTING_NAMESPACE_LABELS",
                (("user", "User"), ("site", "Site")),
            ),
            ChoiceVocab(
                "SettingSource",
                ...
                (("user", "Personal"), ("database", "Database")),
            ),
        ]
    )
    ...
    assert 'export const SETTING_NAMESPACES = ["user", "site"] as const;' in output
    assert 'export const SETTING_SOURCES = ["user", "database"] as const;' in output
    assert "satisfies Record<SettingNamespace, string>;" in output
    assert "satisfies Record<SettingSource, string>;" in output


def test_choice_vocabularies_rejects_empty_list() -> None:
    with pytest.raises(ValueError, match="at least one vocabulary"):
        render_choice_vocabularies([])


def test_choice_vocabularies_rejects_duplicate_exported_names() -> None:
    with pytest.raises(ValueError, match="duplicate exported name 'SETTING_SOURCES'"):
        render_choice_vocabularies(
            [
                ChoiceVocab(
                    "SettingSource",
