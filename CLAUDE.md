...
- **`quick_filter.py`** — `QuickFilterBar()` (#197): the GitHub-style facet row above each list view (wired by `ts/elements/quick-filter-bar.ts`), plus `QUICK_FACETS` and the `is_quick_editable` degrade predicate. A `QuickFacet(dropdown=True)` facet (#315 tryout: every sessions facet) renders as a ghost "Label ▾" `ComboboxDropdown` hosting the panel-layout widget instead of the inline `Label: [widget]` pair — set → panel `FilterSelect`, date → `DateRangePanel` (static calendar), number → `NumberFilter` embedded as-is; string/bool have no panel form yet and raise
