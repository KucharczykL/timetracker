...
- **`quick_filter.py`** — `QuickFilterBar()` (#197): the GitHub-style facet row above each list view (wired by `ts/elements/quick-filter-bar.ts`), plus `QUICK_FACETS` and the `is_quick_editable` degrade predicate. A `QuickFacet(dropdown=True)` set facet (#315 tryout: sessions game/device) renders as a ghost "Label ▾" `ComboboxDropdown` hosting the panel-layout `FilterSelect` instead of the inline `Label: [widget]` pair
