...
- **Prefer the htpy-style call form** — write `Builder(class_="x", hx_get="/y")[child1, child2]` (kwargs attributes + `[]` children; see the htpy-style markup note above), **not** the verbose `Builder(attributes=[("class", "x")], children=[...])`. Reach for the named builder a tag has (`Button`, `A`, `Div`, `Span`…) instead of raw `Element("button", ...)`; if a tag has no builder, add it to the whitelist in `primitives.py` (and export it from `common/components/__init__.py`) rather than hand-rolling `Element(...)` at the call site. If a component genuinely cannot be called this way, file an issue instead of silently falling back to the verbose form.
