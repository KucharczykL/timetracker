...
- **`temporal_field.py`** — `TemporalField()`, the native controls for a date at
  ...
  pages are in `games/views/game.py`: Add Game, whose one inline Release row is
  `InitialReleaseForm`, and Edit Game, whose Editions area hosts one per Release
  row. The grammar, the wire and the no-script contract are in
...
- **A private catalog graph is written through the service** — call
  ...
  hosted by Edit Game: it reads the whole posted set of `EditionRowForm` /
  `ReleaseRowForm` rows, calls the verbs in one order, and puts a refusal back on
  the row that stated it. Its `save()` goes through `write_and_mirror()` from
  `games/catalog_compat.py`, so the flat `platform` / `year_released` columns
  follow the graph in the same transaction. There are no standalone Edition or
  Release routes; #969 folded all six into the Game form. The contract is
