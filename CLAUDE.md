...
- **PlayerGame** — the first projection: one row per catalog game a library tracks, written only by a projector folding `library.playergame.created`. Both `UUIDv7Field` defaults are opted out of, because the pk is the event's `aggregate_id`; `game` is `RESTRICT`, so a projection row is never collateral
