"""One record's external references, as one form."""
...
class ReferenceSetForm(forms.Form):
    """One field per registered provider.

    `target` is None on an Add page; `bind()` names the record the
    submit made. The fields are built after ``super().__init__()``,
    thus out of :class:`PrimitiveWidgetsMixin`'s reach, and the
    stamping call the mixin would have made is here instead.
    """
    ...
    def _stored(self, target: CatalogTarget) -> dict[str, str]:
        """The keys this record holds, by field."""
    ...
    def clean(self) -> dict[str, object]:
        """Each policy's own sentence, on its box."""
    ...
    def write(self) -> None:
        ...
        A raise rather than an `assert`: `python -O` strips one,
        and what is left writes nothing and says nothing.
        """
    ...
    def answer(self, refusal: ValidationError) -> bool:
        """Put the sentence on its own box."""
...
def submitted_or_form_error(
    ...
    """One transaction: the record and its references."""
    try:
