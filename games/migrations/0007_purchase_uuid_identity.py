...
def require_match(path, actual, expected):
    if actual != expected:
        raise RuntimeError(f"PUR identity {path} mismatch: {actual!r} != {expected!r}")
...
def reconcile_purchase_identity(apps):
    Purchase = apps.get_model("games", "Purchase")

    purchase_rows = Purchase.objects.count()
    (
        row_count,
        distinct_count,
        null_count,
        bad_version_count,
        max_delta_ms,
    ) = model_identity_counts(Purchase._meta.db_table)

    require_match("Purchase.row_count", row_count, purchase_rows)
    require_match("Purchase.null_count", null_count, 0)
    require_match("Purchase.distinct_count", distinct_count, purchase_rows)
    require_match("Purchase.bad_version_count", bad_version_count, 0)
    require_match("Purchase.max_timestamp_delta_ms", max_delta_ms, 0)
    require_match("Purchase.order_preserved", order_preserved(Purchase), True)

    per_model_uuids = [set(Purchase.objects.values_list("uuid", flat=True))]
    per_model_uuids += [
    ...
    print(
        "PUR identity backfilled "
        f"purchase_rows={purchase_rows} purchase_distinct={distinct_count} "
        "max_timestamp_delta_ms=0 order_preserved=true"
    )


def backfill_purchase_uuid(apps, schema_editor):
    del schema_editor
    backfill_model_uuids(apps.get_model("games", "Purchase"))
    reconcile_purchase_identity(apps)
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(backfill_purchase_uuid, migrations.RunPython.noop),
        # Every FK in this schema is DEFERRABLE INITIALLY DEFERRED (see 0004's
        # identical use of this statement). If any row referencing purchase was
        # inserted earlier in this same transaction, its deferred RI check is
        # still pending and blocks the ALTER TABLE below with "cannot ALTER
