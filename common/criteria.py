...
class FilterField:
    ...
    # Options for a field that names no column.
...
def _maybe_group_for(model: type[models.Model], column: str) -> ComparisonGroup | None:
    ...
    with no resolvable output type, is a temporal projection generated by #655,
    or is of a type absent from ``_GROUP_BY_INTERNAL_TYPE`` (e.g. AutoField pk,
    JSONField).
...
def _comparison_group_for(model: type[models.Model], column: str) -> ComparisonGroup:
    ...
    (does not exist, is a relation, is a GeneratedField with no output type or a
...
def comparable_columns(model: type[models.Model]) -> list[ComparableColumn]:
    ...
    The pk/AutoField, GeneratedFields without an output type, temporal projection
    GeneratedFields, and JSONField classify to None in ``_maybe_group_for`` and
    are excluded from every block.
...
def field_metadata(filter_cls: type[OperatorFilter]) -> list[FieldMeta]:
    ...
    for dataclass_field in dc_fields(filter_cls):
        ...
        if criterion_cls is not None:
            ...
            # A declared set wins over the column's.
