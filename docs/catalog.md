...
Restoring an owned Game first looks for a live Game of the library that the
two partial unique constraints would refuse: the same name, platform and year,
or, with no platform, the same name and year. A recreated Game refuses the
restore with a sentence naming it, and nothing changes. The check is a
forecast; a Game created after it still meets the constraint.
