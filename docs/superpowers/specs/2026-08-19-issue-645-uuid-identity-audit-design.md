...
is `convalidated`. The second half is the one that is not vacuous on a committed
database — PostgreSQL permits `NOT VALID` constraints, which enforce new rows
while never having checked existing ones. The orphan count is still worth
asserting and, unlike on a schema with immediate constraints, is falsifiable in a
test: every foreign key here is `DEFERRABLE INITIALLY DEFERRED`, so an orphan row
inserted inside a transaction is visible to the audit before the commit that
would reject it.
...
key referring to that model, discovered through
`_meta.get_fields(include_hidden=True)` rather than a hand-written list. The
`games:` many-to-many lists are integer primary keys and are untouched.

`related_objects` is the wrong accessor, and reaching for it would have
reintroduced the exact class of bug this derivation exists to prevent: it filters
out hidden relations, and `UserLibraryPreferences.default_device` is declared
`related_name="+"`. The one `Device` referrer outside the dumped set would have
kept pointing at a pre-reassignment uuid, invisibly — that table is not dumped,
and its deferred foreign-key check falls inside a transaction that never commits.
...
- The other three carry no `uuid` at all — the blob predates migrations `0006`
  and `0007`. On load they receive the field's **Python** default
  (`default=uuid.uuid7`) at load time in file order — not the `db_default`, which
  `save_base(raw=True)` never reaches for a column the deserializer supplies —
  while their `created_at` is date-jittered, so ordering agreement fails there
  and nowhere else. The committed blob is clean only where someone repaired it by
  hand.
...
- **The fixture has two producers that are only coincidentally aligned.**
  ...
  transform, here as in ID-06 and ID-09. So the anonymizer's own tests and the
