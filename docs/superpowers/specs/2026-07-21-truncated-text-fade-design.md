...
- **Scope:** all name/label truncation sites — `NameWithIcon` (linked + menu), `LinkedPurchase` single-game, sort-name column. Menu items (caller-wrapped in `<a>`) get fade + **hover-only** tooltip, no tap button (see Menu contract).
