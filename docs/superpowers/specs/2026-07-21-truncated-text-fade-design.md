...
> Rev 5: reveal button is **`absolute` (out of flow)** over the fade area so it never shrinks the clip — removes the overflow-state hysteresis (stuck-open feedback loop). Added the button-width-threshold growth test; Decisions/menu wording made consistent (hover-only).
> Rev 6: resolves 4 P1 + 2 P2 (review on `080b508`): (1) spike measures the table's own `overflow-x-auto` **scroll wrapper**, not `documentElement`, and the fallback constrains **both** cell table-sizing and child width; (2) ids use `randomid(content=…)` (the `seed=` form returns the raw seed for long names — invalid) with a **context+slot `instance_key`** (navbar renders each session twice), plus a full-page id-uniqueness invariant; (3) a `leading` slot keeps the platform/emulation icons **inside the link** and the focus ring **outside** the clip; (4) unlinked `tap=True` (sort-name) gets an overflow-only, element-managed `tabindex` so desktop keyboard can reveal it; (5) multi-game bundles fold into `TruncatedText` with `reveal="always"` + `tooltip_content=games-list` (no competing popover); (6) `TableRow(data, columns=None)` stays optional (a production call lives outside `StyledTable`), and the controller's `content` is optional (the pop-over test fixture has none). Minor: #455 already closed; Tailwind lockfile is 4.3.0.
...
- **Mobile 390px table (first + last columns only):** `min-w-0` lets the name cell shrink to the width left by the fixed-ish actions column; the name clips/fades to fit. **Acceptance (corrected — P1 #1): the table's own `overflow-x-auto` scroll wrapper (`primitives.py:1757`, `Div(class_="relative overflow-x-auto")`) has `scrollWidth <= clientWidth`** — measuring `documentElement` is wrong because that wrapper absorbs table overflow into an inner scrollbar (a probe showed doc `scrollWidth==390` passing while the wrapper was `580`). The self-cap alone did **not** achieve this in the probe (the host stayed 384px instead of shrinking into the first-column space), so this is expected to require the fallback (below).
...
Confirm the CSS clipping works in the **real** `table-auto` list table (first-column `<th class="… whitespace-nowrap">`, `common/components/primitives.py:1369`) inside its `overflow-x-auto` scroll wrapper. Throwaway page + Playwright measure (like the earlier `test_tmp_*` probes). The probe in review already showed the self-cap **insufficient** (host 384px, wrapper 580px; even `max-width:384px` on the `<th>` alone left the wrapper at 532px), so the spike's job is to find the **complete cell + child constraint set**, empirically, and encode it. **Assert, at 390 / 640 / 768px:**
...
interface TooltipConfig {
  ...
  content?: HTMLElement;  // OPTIONAL inner scroll wrapper — max-height cap target; falls back to panel when absent (P2 #6: the pop-over test fixture has no content wrapper)
...
- **`pop-over.ts` refactors to a thin wrapper** over `attachTooltip`, preserving its `data-pop-over-*` markup so existing selectors keep working. Its #445 vitest (`ts/elements/pop-over.test.ts`) + e2e are the regression net — must stay green (this is why the extraction follows the spike). **Note (P2 #6):** the vitest mount fixture (`pop-over.test.ts:8-24`) renders `data-pop-over-{trigger,panel,arrow}` but **no `data-pop-over-content`** wrapper — so `content` is optional in the controller (above); the fixture stays as-is. The **name-reveal** e2e selectors move from `pop-over [data-pop-over-trigger]` to `truncated-text [data-truncated-reveal]` in `test_touch_targets_e2e.py` + `test_purchase_e2e.py` once names migrate (the "!" cue and price popovers stay on `pop-over`).

### 2. `<truncated-text>` element
- **Python** (`common/components/primitives.py`): `TruncatedText(text, *, instance_key, leading=None, link=None, tap=True, reveal="auto", tooltip_content=None, reveal_label="Show full name", max_width=_NAME_MAX_W)` builder + `_TruncatedText = custom_element_builder("truncated-text")`. Register `class TruncatedTextProps(TypedDict): tap: bool; reveal: str` in `custom_elements.py` (mirror `PopOverProps`, `custom_elements.py:512`); run `make gen-element-types`.
  - **`leading`** (P1 #3): a visible-content slot (the platform/emulation icons) rendered **inside the link, before the clip**, so icons stay within the clickable area. **`reveal`**: `"auto"` (default — reveal gated by overflow) or `"always"` (multi-game bundle: reveal shown regardless of overflow). **`tooltip_content`**: panel body, defaults to `text`; multi-game passes the games-list node. **`instance_key`**: a page-unique context+slot key for the id (see ARIA).
  - **Markup + DOM nesting (P1 #3):** host `<truncated-text tap="…">` is a Tailwind **`group`** (`group relative inline-flex min-w-0 {max_width}`) carrying `data-overflowing`; overflow-reactive children use a **`group-data-[overflowing]:` parent-state selector**, never a bare `data-[overflowing]:` (which matches the child's own attribute, not the host's). Final nesting:
    - **linked:** `<a href>[ leading-icons, <span data-truncated-clip>text</span> ]</a>` then the sibling reveal `<button>` then the panel. The clip's `overflow-hidden` is on the **inner span only**, so the `<a>`'s focus ring (on the link box, outside the clip) is **not clipped**; icons stay inside the link (full hit area).
    - **unlinked (`tap=True`, e.g. sort-name):** `<span data-truncated-clip>[ leading-icons, text ]</span>` + reveal button + panel (no `<a>`).
    - `data-truncated-clip`: `block overflow-hidden whitespace-nowrap min-w-0` + `group-data-[overflowing]:[mask-image:…]`. Reveal `<button data-truncated-reveal>` (`Icon("ellipsis")`, mobile-only `[@media(hover:hover)]:hidden`, shown via `group-data-[overflowing]:` — or always when `reveal="always"`). `data-truncated-panel` tooltip with inner `data-truncated-content` + `data-truncated-arrow`, mirroring `_popover_html` (`primitives.py:271-324`). The reveal button is always a **sibling of the `<a>`** (never nested — #445 invariant, guarded by `test_html_validity`).
  - **Unlinked desktop keyboard reveal (P1 #4):** an unlinked host has no focusable element for desktop keyboard users (the only `<button>` is `hover:hover`-hidden). Fix: the element manages an **overflow-only `tabindex="0"` on the clip span** — added when `data-overflowing` (and no wrapping link) so the clip is focusable and focus opens the tooltip via the controller (`trigger` = the clip span), **removed when it fits** so short names gain **no useless tab stop**. Linked hosts already have the `<a>` (a descendant → focus bubbles to the host); menu (`tap=False`) stays hover-only.
  - **No overflow hysteresis (review):** the reveal button is **`absolute` positioned** over the clip's right-edge fade area (`absolute inset-y-0 right-0` on the host, which is `relative`), **out of normal flow**, so showing/hiding it does **not** change the clip's width — the measurement basis (`clip.scrollWidth` vs `clip.clientWidth`) is independent of the button. Without this, an `inline-flex` sibling button would consume ~24px when shown, keeping the clip narrow enough to stay `data-overflowing` even after the viewport grew past the point where the text fits in a no-button layout (a stuck-open feedback loop). The button overlays the already-faded last ~1.5rem, so it hides no readable text, and remains 24px (WCAG 2.5.8). It is still a **sibling of the `<a>`**, just positioned — the invariant holds.
  - **ARIA / tooltip identity (P1 #2 — server-side, hashed, page-unique):** the panel `id` and `aria-describedby` are rendered **server-side** (the `_popover_html` contract; a11y before JS). The id **must** use the hashing form: `panel_id = randomid(content=f"truncated-text:{instance_key}:{text}")`. The earlier `randomid(seed=…)` form is **broken**: `randomid` (`core.py:470`) only appends `length - len(seed)` hash chars, so a seed ≥10 chars is returned **verbatim** — a name-based seed yields an id like `A Very Long Game:1` (spaces + colon → invalid, and `aria-describedby`, a whitespace-separated IDREF list, won't resolve). The `content=` form always hashes to 10 hex chars.
    - **`instance_key` must be page-unique context+slot, not merely a model pk** — the same session renders into **both** the desktop and mobile navbar (`layout.py:376`/`:537`), a game appears in the table *and* the navbar, and a session list can repeat a game. So callers build keys like `navbar-log-mobile:{session.pk}` vs `navbar-log-desktop:{session.pk}`, `games-list:{game.pk}`, `sessions-list:{session.pk}:name`, `game-detail-sort:{game.pk}`.
    - `aria-describedby={panel_id}` on the visible name element (the `<a>` if linked, else the clip span) **and** on the reveal button when present. `role="tooltip"` on the panel. JS never assigns ids.
    - **New invariant test** (P1 #2): a full-page test asserting every `id` is globally unique and every `aria-describedby` token resolves to exactly one element, with a fixture containing duplicate names **and both navbar copies**. `test_html_validity` currently checks interactive nesting only, so this is new parser logic (or a sibling invariant).
- **TS** (`ts/elements/truncated-text.ts`): `connectedCallback` reads `readTruncatedTextProps` and wires a `ResizeObserver` on the clip. On each measure: `overflowing = clip.scrollWidth > clip.clientWidth`; toggle `data-overflowing` on the host. On the overflowing→not-overflowing transition, **`controller.close()`** so an open tooltip doesn't linger when a resize makes the text fit (review #3), **and remove the managed `tabindex`** from the clip (unlinked case, P1 #4); on the reverse transition, add it. For `reveal="always"`, treat as always-active (reveal + tooltip regardless of overflow; the fade still gates on overflow). Instantiate `attachTooltip({..., isActive: () => this.reveal === "always" || this.overflowing})`. `disconnectedCallback` disconnects the observer + `controller.destroy()`. Native `connectedCallback` (fires on parse + htmx swap) — no `onSwap`.
- **CSS:** fade only under overflow — `group-data-[overflowing]:[mask-image:linear-gradient(to_right,#000_calc(100%-1.5rem),transparent)]` on the clip (host is the `group`; Tailwind v4 — lockfile resolves **4.3.0** — compiles arbitrary `[mask-image:…]` + `group-data-[…]` variants, confirmed). The reveal button's visibility gates the same way (`group-data-[overflowing]:` shown, or always for `reveal="always"`); tooltip activation is JS via the controller's `isActive`.
...
- `NameWithIcon` linked → `TruncatedText(name, link=…, leading=icons, instance_key="…")`; unlinked/menu → `TruncatedText(name, link=None, leading=icons, tap=tap, instance_key="…")` (menu passes `tap=False`, `max_width="max-w-full"`). `leading` carries the platform/emulation icons (`domain.py:222-251`), keeping them inside the link.
- `LinkedPurchase` **single-game** → `TruncatedText(name, link=…, leading=Icon(...), instance_key="…")`. **Multi-game (P2 #5):** also `TruncatedText`, but `reveal="always"` + `tooltip_content=games_list` — the bundle name **width-clips + fades** like any other name, and the reveal button/tooltip is always available showing the games list. This replaces the separate `_reveal_popover` for the bundle case, so there's **no second competing tooltip/button** and no leftover char-truncation for long bundle names.
- Sort-name column (`game.py:125`, unlinked) → `TruncatedText(sort_name, link=None, tap=True, instance_key="game-detail-sort:{game.pk}")` — distinct from the name cell's key on the same row.
...
- `tests/test_touch_targets_e2e.py` + `e2e/test_purchase_e2e.py` (migrate name-reveal selectors from `pop-over` to `truncated-text`); the new full-page id-uniqueness invariant (extend `tests/test_html_validity.py` or a sibling).
...
- **Spike** (Step 0) green before building — the **`overflow-x-auto` scroll wrapper** has `scrollWidth <= clientWidth` at 390px (not `documentElement`), the clip shrank below the 24rem cap, the actions column stays visible, and middle columns reappear at 640/768px.
...
- **Playwright** (`test_truncated_text_e2e.py`):
  ...
  - **unlinked keyboard (P1 #4):** an overflowing **sort-name** (unlinked) is Tab-focusable, focus opens its tooltip, Escape closes it; a short sort-name is not in the tab order;
  - **multi-game bundle (P2 #5):** a long-named multi-game purchase clips + fades the bundle name **and** exposes the games-list reveal (`reveal="always"`) with a single tooltip/button (no competing popover);
  ...
  - **no button-width hysteresis (review):** overflow → button appears → grow the viewport to just past where the text fits the **no-button** layout → `data-overflowing` clears at that threshold (not a button-width later), and fade+button disappear — proving the absolute button doesn't hold the state open;
  - **id uniqueness (P1 #2):** a full-page fixture with duplicate names **and both navbar copies** (desktop + mobile) — every `id` is globally unique, every `aria-describedby` token resolves to exactly one element, all present in the initial server HTML (before JS);
