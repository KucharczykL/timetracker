...
submit for each act, each with its own `formaction`. One form, because a
...
`<selection-actions>` asks `<selectable-table>` for the statement at connect,
and reads `selectable-table:change` on that host after. The table announces a
restored selection from its own connect, before this element upgrades, so a
statement only announced is a statement missed. The element writes the
...
session names, refuse this way. A row of another library is a defect and ends
the batch. The inverse is the restore, which refuses a session a live record
was made from.
...
beside it. The Library page's Playtime section keeps its prose, its count and
the link to the review, and presses nothing. The Actions columns stand until
#718.

Vitest covers the statement the slot posts, a selection restored before the
slot upgrades, and the press that stops the writing. Pytest covers each act's
scope, resolve, run and inverse, and the confirmation each preview renders.
Three passes drive a real browser: two rows removed and put back, a refused
row beside removed siblings, and the runner's walk over two chunks.
