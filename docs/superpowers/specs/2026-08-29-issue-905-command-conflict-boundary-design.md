...
exists. This issue moves it to a module that names no domain, completes it, and
adds the guards that keep it complete.

## The translation is incomplete today

`CommandConflict` has three subclasses, not two.
...
`_translated` answers the first two. The third reaches a person as a 500.

It is unreachable through `dispatch` today. `LockedStream.append` raises it only
for a caller that passes `expected_sequence`, `idempotent_append` passes none,
and the one caller that does — `games/events/rebuild.py` — catches its own. The
answer is therefore written against a later command rather than against a
current defect. It is written now because the guard below refuses an unanswered
leaf, and because a leaf that is answered when it appears needs no second
memory.

## Where the translation lives

`games/writes/answers.py` is a new module. It holds the answer type, the
sentences and the context manager that applies them. The name states what the
module makes: `games/writes/playergame.py` already opens with "State a fact;
answer a refused one".
...
`_translated` becomes `answered`, and takes the subject noun. A call site reads
`with answered("game"):`.
...
The first, second, third and fifth answers are the ones #677 already gives. No
wording changes, and no status code changes.
...
change to what a person sees, thus it is not this issue's to make. The layering
...
The answers are `except` clauses today. The three conflict leaves become entries
in a mapping keyed on the exception type.
...
mapped leaf is answered by its parent.

`CommandNotPermitted` and `CommandRejected` stay as fixed clauses. Neither is a
...
attempt would complete. Neither is better than a failure that is reported as
one.

## The guards

Two tests, because two things can go wrong.

**Every leaf of the hierarchy is answered.** The test imports every module of
the `games` package with `pkgutil.walk_packages`, collects the subclasses of
`CommandConflict` at any depth, and asserts each is in `CONFLICT_ANSWERS`.

The walk is what makes the test worth writing. `__subclasses__` sees a class
only after its module is imported, and `games/writes/answers.py` imports exactly
the modules whose leaves it maps. A guard that reads only its own import graph
...
**Every exception of the boundary is classified.** The second test collects
every `Exception` subclass defined in `games/events/conflicts.py`, `dispatch.py`,
...
`TransactionRequired` and `PayloadNotCanonical`. Each carries its reason.
...
answer `bool`. `games/views/purchase.py:579` needs a status anyway, so it
restates the value as the literal `409`.
...
The alternative changes what a person sees, thus it is refused above. The cost
is recorded here and in a comment on the clause, so a later reader finds a
...
No schema changes. No data changes. No user-visible behavior changes, because
the one answer this issue adds is for a leaf that `dispatch` cannot raise. The
commit is reverted by itself.

## Verification

- Each of the three conflict leaves answers with its sentence and 409, and the
  subject noun is interpolated.
- Every mapped sentence interpolates with a probe subject and leaves no brace
  behind, thus a mistyped placeholder fails a test rather than a request.
...
- A `CommandConflict` subclass absent from the mapping leaves `answered`
  unchanged.
- A subclass of a mapped leaf is answered by its parent's entry.
- The hierarchy guard fails when a leaf is unmapped. Proven by removing an entry
  in the test itself, not by trusting the walk.
- The boundary guard fails when an exception of those five modules is in none of
  the three sets.
...
- Through the API, a refused status change is answered 409 with a `detail` body,
  which proves the exception handler follows the rename.
...
- A rendered page for a conflict. A view answers a form post with a message on
  ...
  htmx must not swap. Whether a conflict deserves a page of its own is a
