...
Three modules render their form again after a refused command. Each answers the
refusal's status: `edit_game` in `games/views/game.py`, both session forms in
`games/views/session.py`, and both historical playtime forms in
`games/views/historical_playtime_entry.py`. A status states what the request
did, and the request did not save.
