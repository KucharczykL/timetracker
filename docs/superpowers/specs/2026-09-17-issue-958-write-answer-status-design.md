...
`CommandFailed` carries a sentence and a status code. A view that stays on its
page cannot read the second one. The six request-shaped write wrappers catch the
exception, raise a message from it, and answer `bool`: the sentence reaches the
person as a toast and the status stops at that line.
...
The two wrappers that raise instead of answering — `remove_game_for_request` and
`remove_run_for_request` — keep raising. A view behind a confirmation hands the
exception to `confirm_and_apply`, which reads the status already, and a
`WriteAnswer` there would ask the confirmation to re-raise what it was given.
...
moves: `HTMXMessagesMiddleware` attaches the toast header to every answer that is
not a redirect, and htmx swaps nothing outside 2xx, which is why the view answers
a status rather than a redirect in the first place. 409 and 500 differ only in
what the network record says, and a defect of ours reading as a conflict the
person can retry is the thing this issue removes.

Two other HTML failure paths keep their numbers. A refused add redirects with
302, and a refused edit renders its form again with 200. Each states where the
person lands, which no refusal knows; the refund's number is the only one a
refusal could ever supply.
...
and fails at 500, which is the hole this issue names.
