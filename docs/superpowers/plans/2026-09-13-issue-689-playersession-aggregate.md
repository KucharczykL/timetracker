...
- **`answers.py` has no answer for a database refusal.** Closed after this wave: `answered()` catches every `django.db.Error`, logs the constraint name and answers `REFUSED_BY_DATABASE` with status 500. This wave still keeps every CHECK looser than its command, because the backstop can only say that nothing was saved.
