...
domain value plus three persisted generated columns.
...
- Produces Django expressions `TemporalLowerBound(expression)`,
  `TemporalUpperBound(expression)`, and `TemporalPrecisionValue(expression)`
  with typed output fields.

The public SQL functions use two private immutable atom functions with exact
roles:
...
single atom           -> atom lower, atom upper, atom precision
...
- [ ] **Step 3: Add migration `0017_temporal_value_domain`.** Create the two
  private atom functions named above, then the lower/upper/precision public
  functions, then `timetracker_temporal_is_valid`, then the domain. Reverse in
  the exact opposite order. Implement `is_valid` as an immutable PL/pgSQL
  exception boundary that calls all three public projections and returns false
  for any rejected value; the domain check is
...
- [ ] **Step 5: Write failing Django-field integration tests.** Under
  ...
  `TemporalValueField` and three persisted `GeneratedField`s using the public
  ...
  generated values, filters over lower/upper/precision without canonical-text
  expressions in their SQL, and raw invalid insert rejection.
