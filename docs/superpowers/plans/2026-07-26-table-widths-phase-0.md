...
|---|---|---|
| `common/components/primitives.py` | Owns `Column`, the shrinkable class constant, and both `<th>` emitters | Modify: `Column` (~1641), `_header_cell` (~1956), `TableRow` (~1688). The constant stays unexported, matching `NAME_MAX_WIDTH_CLASS`. |
| `games/views/session.py` | Sessions list columns | Modify line 96 |
| `games/views/game.py` | Games list + game-detail purchases table columns | Modify lines 111, 634 |
| `games/views/purchase.py` | Purchases list columns | Modify line 177 |
| `tests/test_components.py` | Unit coverage for the flag | Modify `test_first_column_class_reaches_header_and_body_cell` (1760), `test_direct_table_row_keeps_columns_optional` (1774) |
| `e2e/test_truncated_text_e2e.py` | Browser coverage at real widths | Modify `test_table_constraints_hold_at_mobile_and_intermediate_widths` (184); add one dead-space test |
...
- [ ] **Step 2: Confirm the tree is clean and the spec commits survived**
...
**Files:**
- Modify: `common/components/primitives.py` (`Column` ~1641, `_header_cell` ~1956, `TableRow` ~1688)
- Test: `tests/test_components.py` (~1760)
...
Replace `test_first_column_class_reaches_header_and_body_cell` (currently at `tests/test_components.py:1760`) and `test_direct_table_row_keeps_columns_optional` (1774) with these four tests. Keep them in the same class, in this order:
...
In `common/components/primitives.py`, directly above `NAME_MAX_WIDTH_CLASS` (~line 476):
...
- [ ] **Step 4: Add the field to `Column`**

In the `Column` NamedTuple (~1641), append the field and extend the docstring's last sentence:
...
- [ ] **Step 5: Emit from the header cell**

In `_header_cell` (~1959), after the existing `column.class_` branch:
...
- [ ] **Step 6: Emit from the row-header cell**

In `TableRow` (~1710), replace the `column_class` assignment:
...
The `if columns` guard is already the established pattern here and keeps the `columns=None` path (used by `tests/test_components.py:1775` and `tests/test_session_row.py:30`) working.

- [ ] **Step 7: Do NOT export the constant**
...
**Files:**
- Modify: `games/views/session.py:96`, `games/views/game.py:111`, `games/views/game.py:634`, `games/views/purchase.py:177`
...
- [ ] **Step 2: Convert the three sortable Name columns**

`games/views/session.py:96`, `games/views/game.py:111`, `games/views/purchase.py:177` each become:

```python
            Column("Name", "name", shrinkable=True),
```

- [ ] **Step 3: Convert the static Name column**

`games/views/game.py:634` (the game-detail purchases table, which has no sort key) becomes:

```python
            Column("Name", shrinkable=True),
```

- [ ] **Step 4: Verify the literal is gone**

Run: `grep -rn 'w-full max-w-0' --include=*.py .`
Expected: no output.

- [ ] **Step 5: Run the page-level suites**
...
**Files:**
- Modify: `e2e/test_truncated_text_e2e.py` (`test_table_constraints_hold_at_mobile_and_intermediate_widths`, ~184)
...
- [ ] **Step 1: Add the dead-space helper**

Add near `_center_x` (~line 53):
...
- [ ] **Step 3: Add the desktop dead-space test**
...
If `test_desktop_name_column_has_no_dead_space` fails with a large positive number, Task 2 did not take effect — re-check `grep -rn 'w-full max-w-0'`.
...
- [ ] **Step 2: Write the numbers into the spec**
...
- [ ] **Step 3: Confirm the scratch files never got committed**
...
Then open a PR whose body states: the measured before/after (sessions at a 1217px container, name cell 551px → 234px, dead space 247px → 0, DATE unwrapped), that the 768 e2e branch was deliberately repealed with the measured 0 → 132px overflow, and `Part of #523`.
...
**Spec coverage.** Phase 0 of the spec asks for: the `shrinkable` field (Task 1), the four call-site conversions (Task 2), the repealed 768 e2e branch with its measured 0 → 132px (Task 3), replacement of the three literal-string assertions at `tests/test_components.py:1770`/`:1772`/`:1777` (Task 1, Step 1), and the recorded 1024 interim behaviour (Task 4). All covered. Phases 1–4 and the refund change are explicitly out of scope and called out at the top.
